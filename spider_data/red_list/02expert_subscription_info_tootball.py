...
# 足球
def expert_subscription_info_tootball():
    url = "http://cpapi.donggeqiu.com/api/zjtj.action"
    # historyPlanList
    with open("02expert_subscription_info_tootball.txt", "r", encoding="utf-8") as f:
        ...
        for i in range(data_count):
            ...
            red_list_football = expert_subscription_info(result, red_list_type,"historyPlanList")
            print("json", str(red_list_football).replace("'", '"'))
            inserDBpersonRedTypeList(uid, name, red_list_type, str(red_list_football).replace("'", '"'))


if __name__ == '__main__':
    expert_subscription_info_tootball()
